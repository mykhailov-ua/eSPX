...
func TestDLQBackupMetrics(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		events[i] = &pb.AdDLQEvent{
			OriginalEvent: &pb.AdStreamEvent{
				ClickId:       ingestion.UnsafeBytes(fmt.Sprintf("click_%d_%s", i, uuid.New().String())),
				CampaignId:    cid[:],
			...
			OriginalId:   ingestion.UnsafeBytes(fmt.Sprintf("msg_%d", i)),
	...
	for _, ev := range events {
		...
		record := map[string]interface{}{
			"id":             ingestion.UnsafeString(ev.OriginalId),
			"format":         "protobuf_dlq",
			"error":          ingestion.UnsafeString(ev.Error),
			"original_id":    ingestion.UnsafeString(ev.OriginalId),
			"failed_at_unix": ev.FailedAtUnix,
			"worker_id":      ingestion.UnsafeString(ev.WorkerId),
			"retry_count":    ev.RetryCount,
			"original_event": map[string]interface{}{
				"click_id":        ingestion.UnsafeString(ev.OriginalEvent.ClickId),
				"campaign_id":     campUUID.String(),
				"event_type":      ingestion.UnsafeString(ev.OriginalEvent.EventType),
				"payload":         ingestion.UnsafeString(ev.OriginalEvent.Payload),
				"ip":              ingestion.UnsafeString(ev.OriginalEvent.Ip),
				"ua":              ingestion.UnsafeString(ev.OriginalEvent.Ua),
