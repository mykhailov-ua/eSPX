...
func TestScanAndRegister(t *testing.T) {
	...
	err = os.MkdirAll("var/ml/artifacts", 0755)
	require.NoError(t, err)

	// 2. Write dummy model.txt and metadata.json
	modelContent := "test model content"
	err = os.WriteFile("var/ml/artifacts/model.txt", []byte(modelContent), 0644)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = os.WriteFile("var/ml/artifacts/metadata.json", metaBytes, 0644)
	require.NoError(t, err)
