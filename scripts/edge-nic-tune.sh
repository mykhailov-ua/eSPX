#!/usr/bin/env bash
# Ingress NIC RX ring and IRQ spread. Usage: edge-nic-tune.sh apply|verify|report|install-systemd
set -euo pipefail
...
install_systemd() {
	...
	install -m 0755 "$ROOT/scripts/edge-nic-tune.sh" /usr/local/bin/edge-nic-tune.sh
...
-h | --help)
	cat <<EOF
Usage: edge-nic-tune.sh <apply|verify|report|install-systemd>
  ...
  install-systemd  install oneshot unit + /usr/local/bin/edge-nic-tune.sh
