...
# Usage: redis_migrate_campaign.sh <campaign_uuid> [source_shard] [target_shard]
...
if [ -z "$SOURCE_SHARD" ]; then
	SOURCE_SHARD="$(cd "$ROOT" && go run "$SCRIPTS/redis/campaign_shard.go" "$CAMPAIGN_ID" "$SHARD_COUNT")"
fi
