#!/usr/bin/env bash
# Edge ingress canary rollout: Phase 0 preflight → 48h soak sign-off → redis reconcile. Usage: edge_rollout.sh <preflight|canary-start|canary-signoff|post-deploy>
set -euo pipefail
...
preflight() {
	bash "$SCRIPTS/edge/edge_phase0.sh" "$ENV_FILE"
}

canary_start() {
	mkdir -p "$BASELINE_DIR"
	STRICT=1 bash "$SCRIPTS/edge/edge_baseline.sh" snapshot
...
canary_signoff() {
	...
	STRICT=1 bash "$SCRIPTS/edge/edge_baseline.sh" verify
...
post_deploy() {
	bash "$SCRIPTS/redis/redis_reconcile_post_deploy.sh" "$ENV_FILE"
	STRICT=1 bash "$SCRIPTS/edge/edge_baseline.sh" verify
