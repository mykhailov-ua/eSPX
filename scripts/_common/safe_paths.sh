...
# Source after paths.sh: source "$SCRIPTS/_common/safe_paths.sh"
...
# buf.gen.yml: out must be a relative staging dir (never ., .., /, or absolute).
safe_validate_buf_gen_yml() {
	local f="$1"
	[[ -f "$f" ]] || safe_die "buf.gen.yml missing: $f"
	local out
...
# sqlc.yml: every out must be internal/<svc>/db without traversal.
safe_validate_sqlc_yml() {
	local f="$1"
	[[ -f "$f" ]] || safe_die "sqlc.yml missing: $f"
	local out
	while IFS= read -r line || [[ -n "$line" ]]; do
		[[ "$line" =~ ^[[:space:]]*out:[[:space:]]*\"?([^\"# ]+)\"? ]] || continue
		out="${BASH_REMATCH[1]}"
		...
		[[ "$out" == */db ]] || safe_die "sqlc out must end with /db: $out"
...
safe_validate_codegen_configs() {
	safe_assert_repo_root
	safe_validate_buf_gen_yml "$ROOT/api/buf.gen.yml"
	safe_validate_sqlc_yml "$ROOT/sqlc.yml"
}
