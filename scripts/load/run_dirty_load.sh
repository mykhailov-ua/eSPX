...
#
# Usage:
#   bash scripts/load/run_dirty_load.sh              # constrained 2k RPS × 5m (default)
#   bash scripts/load/run_dirty_load.sh smoke        # 500 RPS × 2m
#   CONSTRAINED=0 bash scripts/load/run_dirty_load.sh full  # full compose, 5k RPS
#   PREPARE=1 bash scripts/load/run_dirty_load.sh
#
# Env:
#   CONSTRAINED=1  use docker-compose.load-test.yml (default)
...
if [[ "${SNAPSHOT_RUNTIME:-1}" == "1" ]]; then
	bash scripts/load/snapshot_runtime.sh "$OUT/runtime-pre" 5 &
	SNAP_PID=$!
fi

run_k6() {
	local k6_img="grafana/k6:latest"
	local script="$ROOT/scripts/load/k6_dirty_traffic.js"
	if [[ "$K6_DOCKER" == "1" ]]; then
...
bash scripts/load/analyze_bottlenecks.sh "$OUT"
