#!/usr/bin/env bash
# Host tuning for local k6 dirty-traffic load tests: sysctl, ulimit checks, preflight report.
# Usage: host_tune.sh apply|verify|report
#
# Requires root for apply. Does not modify docker-compose or k6 — see HOST_LOAD_TEST_TUNING_REPORT.md.
set -euo pipefail
