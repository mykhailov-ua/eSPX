...
if [ ! -f "$ENV_FILE" ]; then
	echo "redis-reconcile-post-deploy: missing env file: $ENV_FILE" >&2
	exit 1
fi
...
if [ "${#ADDRS[@]}" -ne "$SHARD_COUNT" ]; then
	echo "redis-reconcile-post-deploy: expected $SHARD_COUNT shards, got ${#ADDRS[@]}" >&2
	exit 1
fi
...
echo "redis-reconcile-post-deploy: checking config:version on ${#ADDRS[@]} shards..."
REF_VERSION=""
for i in "${!ADDRS[@]}"; do
...
echo "redis-reconcile-post-deploy: checking config:values field count..."
REF_FIELDS=""
for i in "${!ADDRS[@]}"; do
...
echo "redis-reconcile-post-deploy: checking blacklist:manual SCARD..."
REF_BL=""
for i in "${!ADDRS[@]}"; do
...
echo "redis-reconcile-post-deploy: checking blacklist:fraud SCARD..."
REF_FRAUD=""
for i in "${!ADDRS[@]}"; do
...
if [ "$FAIL" -ne 0 ]; then
	echo "redis-reconcile-post-deploy: DRIFT detected — run management cold sync or scripts/redis-migrate-campaign.sh" >&2
	exit 1
fi

echo "redis-reconcile-post-deploy: OK"
