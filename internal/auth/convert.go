...
// normalizeEmail lowercases and trims mailbox addresses for consistent lookups.
func normalizeEmail(email string) string {
	return strings.ToLower(strings.TrimSpace(email))
}

// toPgUUID converts a google/uuid value into pgtype representation.
func toPgUUID(u uuid.UUID) pgtype.UUID {
	return pgtype.UUID{Bytes: u, Valid: true}
}

// uuidFromPg extracts a google/uuid from pgtype storage.
