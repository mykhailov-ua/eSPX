...
func campaignSyncKey(id uuid.UUID) string {
	return campaignHashTag(id) + "budget:sync:campaign:" + id.String()
}

func customerSyncKey(campaignID, customerID uuid.UUID) string {
	return campaignHashTag(campaignID) + "budget:sync:customer:" + customerID.String()
}

func fcapKeyPrefix(campaignID uuid.UUID, brandFcapKey string) string {
	if brandFcapKey != "" {
		return brandFcapKey + ":u:"
	}
...
func RedisClusterSlot(key string) int {
	...
	for i := 0; i < len(key); i++ {
		if key[i] == '{' {
			tagStart = i
		} else if key[i] == '}' && tagStart >= 0 {
			tagEnd = i
			break
		}
	}
	hashKey := key
	if tagStart >= 0 && tagEnd > tagStart+1 {
