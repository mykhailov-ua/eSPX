...
// MLModelVersionPayload represents the payload for ML_MODEL_VERSION outbox events.
type MLModelVersionPayload struct {
	ModelVersion string `json:"model_version"`
	Hash         string `json:"hash"`
	ShardID      int    `json:"shard_id"`
}

// MLModelSyncOrchestrator coordinates rolling model deployment across Redis shards.
type MLModelSyncOrchestrator struct {
	svc *Service
}

// NewMLModelSyncOrchestrator constructs a new orchestrator.
func NewMLModelSyncOrchestrator(svc *Service) *MLModelSyncOrchestrator {
	return &MLModelSyncOrchestrator{svc: svc}
}

// Tick runs one cycle of the rolling model sync state machine.
func (o *MLModelSyncOrchestrator) Tick(ctx context.Context) error {
	pool := o.svc.GetPool()
	if pool == nil {
	...
	if activeSyncShard != -1 {
		...
		if time.Since(state.startedAt) > 180*time.Second {
			...
			slog.Error("ML model sync timed out on shard, triggering rollback", "shard_id", activeSyncShard, "version", versionID)
		...
		if err != nil {
			slog.Warn("ML model sync canary check failed with error, rolling back", "shard_id", activeSyncShard, "version", versionID, "error", err)
			return o.rollbackShard(ctx, activeSyncShard, versionID)
		}

		if passed {
			// Canary OK -> CUTOVER to ACTIVE
			slog.Info("ML model sync canary passed, cutting over shard to ACTIVE", "shard_id", activeSyncShard, "version", versionID)
			...
			slog.Warn("ML model sync canary failed (high FP rate), rolling back", "shard_id", activeSyncShard, "version", versionID)
			return o.rollbackShard(ctx, activeSyncShard, versionID)
		}
	...
	if nextShardToSync != -1 {
		...
		slog.Info("ML model sync starting on shard", "shard_id", nextShardToSync, "version", versionID)
		_, err = pool.Exec(ctx, `
		...
		payload, err := cold.MarshalJSON(MLModelVersionPayload{
	...
	slog.Info("ML model sync complete on all shards, activating version globally", "version", versionID)
...
func (o *MLModelSyncOrchestrator) rollbackShard(ctx context.Context, shardID int, versionID string) error {
	pool := o.svc.GetPool()
...
func (o *MLModelSyncOrchestrator) runCanaryCheck(ctx context.Context, shardID int, versionID string) (bool, error) {
	...
	slog.Info("ML model sync canary stats", "shard_id", shardID, "total_rows", totalRows, "high_scores", highScores, "fp_rate", fpRate)
...
// CheckAndHandleStaleEpochs checks if any Redis shard has a stale ML channel and tightens suspect rate limits if so.
