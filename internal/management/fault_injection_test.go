...
func TestChaos_PGDeadlockRecovery(t *testing.T) {
	...
	_, err := pool.Exec(ctx, `
		...
		($2, 'Deadlock B', 100000000, 'USD')`, repo.ToUUID(id1), repo.ToUUID(id2))
	require.NoError(t, err)
	...
	run := func(first, second uuid.UUID) {
		...
		e := pgx.BeginFunc(ctx, pool, func(tx pgx.Tx) error {
			if _, e := tx.Exec(ctx, `SELECT 1 FROM customers WHERE id = $1 FOR UPDATE`, repo.ToUUID(first)); e != nil {
				return e
			}
			time.Sleep(150 * time.Millisecond)
			_, e := tx.Exec(ctx, `SELECT 1 FROM customers WHERE id = $1 FOR UPDATE`, repo.ToUUID(second))
			return e
		})
		if first == id1 {
			err1 = e
	...
	var bal1, bal2 int64
	require.NoError(t, pool.QueryRow(ctx, `SELECT balance FROM customers WHERE id = $1`, repo.ToUUID(id1)).Scan(&bal1))
	require.NoError(t, pool.QueryRow(ctx, `SELECT balance FROM customers WHERE id = $1`, repo.ToUUID(id2)).Scan(&bal2))
...
func TestChaos_ScheduleTickRace(t *testing.T) {
	...
	require.NoError(t, pool.QueryRow(ctx, `
		SELECT COUNT(*) FROM campaign_status_history WHERE campaign_id = $1`, repo.ToUUID(campID),
	).Scan(&historyCount))
...
func TestChaos_ConcurrentBalanceDepletion(t *testing.T) {
	...
	var balance int64
	require.NoError(t, pool.QueryRow(ctx, `SELECT balance FROM customers WHERE id = $1`, repo.ToUUID(customerID)).Scan(&balance))
...
func TestChaos_AutoscaleInsufficientTotalBudget(t *testing.T) {
	...
	_, err = pool.Exec(ctx, `
		...
		repo.ToUUID(lowCTR), repo.ToUUID(highCTR))
	require.NoError(t, err)

	_, err = pool.Exec(ctx, `UPDATE campaigns SET current_spend = $1 WHERE id = $2`, int64(94_000_000), repo.ToUUID(lowCTR))
	require.NoError(t, err)

	queries := db.New(pool)
	syncWorker := adssync.NewSyncWorker(rdb, repo.NewCampaignRepo(queries), repo.NewCustomerRepo(queries), 100*time.Millisecond)
	...
	require.NoError(t, pool.QueryRow(ctx, `SELECT budget_limit FROM campaigns WHERE id = $1`, repo.ToUUID(lowCTR)).Scan(&limitLow))
	require.NoError(t, pool.QueryRow(ctx, `SELECT budget_limit FROM campaigns WHERE id = $1`, repo.ToUUID(highCTR)).Scan(&limitHigh))
	require.NoError(t, pool.QueryRow(ctx, `SELECT current_spend FROM campaigns WHERE id = $1`, repo.ToUUID(lowCTR)).Scan(&spend))
