...
func TestTrackerDepGraphExcludesMLRuntime(t *testing.T) {
	out, err := runAtModuleRoot(t, "go", "list", "-deps", "./cmd/tracker")
	require.NoError(t, err, string(out))

	deps := string(out)
	assert.NotContains(t, deps, "espx/internal/mlanalytics")
	assert.NotContains(t, deps, "github.com/zhongdai/go-lgbm")
	assert.NotContains(t, deps, "onnxruntime")
}

func TestMLAnalyticsBuilds(t *testing.T) {
	out, err := runAtModuleRoot(t, "go", "test", "-c", "-o", os.DevNull, "./internal/mlanalytics")
	require.NoError(t, err, "mlanalytics build failed: %s", string(out))
}

func TestTrackerBuildsWithoutMLAnalytics(t *testing.T) {
