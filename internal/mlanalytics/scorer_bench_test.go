...
func BenchmarkLGBMScorer_ScoreBatch10k(b *testing.B) {
	if testing.Short() {
		b.Skip("skipped in -short; run manually: go test -bench=BenchmarkLGBMScorer_ScoreBatch10k -benchtime=1x ./internal/mlanalytics")
	}
...
func TestLGBMScorer_ScoreBatch10k_under2s(t *testing.T) {
	if testing.Short() {
		t.Skip("skipped in -short; manual gate: go test -run TestLGBMScorer_ScoreBatch10k_under2s ./internal/mlanalytics")
	}
