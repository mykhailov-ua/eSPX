...
func TestChaos_AdsPGKillOpensConsumerCircuit(t *testing.T) {
	...
	producer := broker.NewStreamProducer(infra.Redis, stack.Stream, 1000, 1*time.Second)
	ctx := context.Background()
	for i := 0; i < 8; i++ {
	...
	assert.Equal(t, processor.CircuitOpen, stack.Consumer.CircuitBreakerState())
}
...
func TestChaos_AdsRedisStopStartTrackRecovery(t *testing.T) {
	...
	requireRedisSteadyStateMetrics(t)
	filter.AssertBudgetInvariant(t, context.Background(), infra.Pool, infra.Redis, stack.CampaignID)
...
func restartAdsIngestStack(t *testing.T, infra *adsChaosInfra, stack *adsIngestStack) *adsIngestStack {
	...
	filterEngine := filter.NewFilterEngine(time.Duration(stack.cfg.FilterTimeoutMs)*time.Millisecond, unifiedFilter)
