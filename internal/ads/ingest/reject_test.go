...
func TestAdsPacketHandler_FilterErrors(t *testing.T) {
	...
	t.Run("ErrBidFloorNotMet -> 402", func(t *testing.T) {
		h := NewAdsPacketHandler(cfg, registry, filter.NewFilterEngine(0, &errFilter{err: filter.ErrBidFloorNotMet}), nil, nil, sharder, "fraud", nil)
		status, written := PostTrackGnetJSON(h, makeBody())
	...
	t.Run("filter timeout -> 504", func(t *testing.T) {
		h := NewAdsPacketHandler(cfg, registry, filter.NewFilterEngine(50*time.Millisecond, &slowFilter{delay: 200 * time.Millisecond}), nil, nil, sharder, "fraud", nil)
		status, written := PostTrackGnetJSON(h, makeBody())
	...
	t.Run("ErrFraudDetected -> 202 silent accept", func(t *testing.T) {
		rdb := &mockRedisXAdd{}
		h := NewAdsPacketHandler(cfg, registry, filter.NewFilterEngine(0, &errFilter{err: filter.ErrFraudDetected}), nil, []redis.UniversalClient{rdb}, sharder, "fraud-stream", nil)
		status, written := PostTrackGnetJSON(h, makeBody())
	...
	t.Run("redis circuit open -> 503 Retry-After", func(t *testing.T) {
		h := NewAdsPacketHandler(cfg, registry, filter.NewFilterEngine(0, &errFilter{err: database.ErrRedisCircuitOpen}), nil, nil, sharder, "fraud", nil)
		status, written := PostTrackGnetJSON(h, makeBody())
	...
	t.Run("ErrRateLimitExceeded -> 429 Retry-After", func(t *testing.T) {
		h := NewAdsPacketHandler(cfg, registry, filter.NewFilterEngine(0, &errFilter{err: filter.ErrRateLimitExceeded}), nil, nil, sharder, "fraud", nil)
		status, written := PostTrackGnetJSON(h, makeBody())
