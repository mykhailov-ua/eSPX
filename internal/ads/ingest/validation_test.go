...
// Guards packet handler rejects invalid HTTP, oversize body, and bad campaign ID via OnTraffic.
func TestAdsPacketHandler_Validation(t *testing.T) {
	...
	t.Run("POST /unknown -> 404 Not Found", func(t *testing.T) {
		...
		assert.True(t, bytes.Equal(conn.Written(), filter.RespNotFound), "expected response: %q, got: %q", string(filter.RespNotFound), string(conn.Written()))
	})
	...
	t.Run("GET /track -> 405 Method Not Allowed", func(t *testing.T) {
		...
		act := handler.OnTraffic(conn)
		assert.Equal(t, gnet.None, act)
		assert.True(t, bytes.Equal(conn.Written(), filter.RespMethodNotAllowed))
	})
	...
	t.Run("POST /track too large -> 413 Payload Too Large", func(t *testing.T) {
		...
		assert.True(t, bytes.Equal(conn.Written(), filter.RespPayloadTooLarge))
	})
	...
	t.Run("POST /track missing Content-Length -> 400 Bad Request", func(t *testing.T) {
		...
		act := handler.OnTraffic(conn)
		assert.Equal(t, gnet.Close, act)
		assert.True(t, bytes.Equal(conn.Written(), filter.RespBadRequestClose))
	})
