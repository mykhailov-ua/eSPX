...
func (h *AdsPacketHandler) parseTrackIngest(
	...
	contentType := repo.UnsafeString(req.ContentType)
	if contentType == "application/x-protobuf" || contentType == "" {
		...
		if err := pbReq.UnmarshalVT(req.Body); err != nil {
			return fields, filter.RespInvalidProto, http.StatusBadRequest, false
		}

		if len(pbReq.CampaignId) != 16 {
			return fields, filter.RespInvalidCampaign, http.StatusBadRequest, false
		}
		copy(fields.campaignID[:], pbReq.CampaignId)

		fields.eventType = repo.UnsafeString(pbReq.EventType)
		if pbReq.Metadata != nil {
			fields.userID = repo.UnsafeString(pbReq.Metadata.UserId)
			if len(pbReq.Metadata.ClickId) > 0 {
				fields.clickID = repo.UnsafeString(pbReq.Metadata.ClickId)
			}
	...
	if err := ParseTrackRequestJSON(trackReq, req.Body); err != nil {
		return fields, filter.RespInvalidJSON, http.StatusBadRequest, false
	}
...
func (h *AdsPacketHandler) deliverGnetTrack(
	...
	case trackStatusFraudAccepted:
		...
		processor.EnqueueFraudReject(h.fraudWriter, shard, evt)
