...
// CampaignMeta is re-exported from rtbbridge for balancer callers.
type CampaignMeta = rtbbridge.CampaignMeta
...
func (hb *HybridBalancer) SelectAndShard(userID string, currentCampaignRps int64) (*CampaignMeta, int) {
	...
	if !isHot {
		shard = int(sharding.JumpHash(uint64(sharding.Crc32Castagnoli(&campaign.ID)), int32(hb.totalShards)))
	} else {
		...
		combinedHash := uint64(sharding.Crc32Castagnoli(&campaign.ID)) ^ uint64(subShardIdx)
		shard = int(sharding.JumpHash(combinedHash, int32(hb.totalShards)))
	}
