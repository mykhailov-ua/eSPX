...
type adsIngestStack struct {
	...
	Consumer      *processor.StreamConsumer
	Registry      *catalog.Registry
	UnifiedFilter *filter.UnifiedFilter
...
func newChaosRegistry(t *testing.T, queries db.Querier) *catalog.Registry {
	t.Helper()
	r := catalog.NewRegistry(queries)
	r.SetReplicaPath(filepath.Join(t.TempDir(), "campaigns_replica.json"))
	return r
}

func seedChaosCampaign(t *testing.T, infra *adsChaosInfra, registry *catalog.Registry) uuid.UUID {
	t.Helper()
	ctx := context.Background()
...
func startAdsIngestStackOpts(t *testing.T, infra *adsChaosInfra, stream string, opts adsIngestStackOpts) *adsIngestStack {
	...
	registry.SetBudgetWarmer(catalog.NewBudgetCacheWarmer([]redis.UniversalClient{infra.Redis}, sharder))
	campaignID := seedChaosCampaign(t, infra, registry)

	store := processor.NewPostgresStore(infra.Queries, 1*time.Second)
	campaignRepo := repo.NewCampaignRepo(infra.Queries)
	...
	consumer := processor.NewStreamConsumer(store, infra.Redis, stream, stream+"-group", stream+"-c1",
