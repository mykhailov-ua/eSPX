...
// SetRefreshPaused stops background cachedUnixMilli/cachedNowUTC refresh (tests only).
func SetRefreshPaused(paused bool) {
	clockRefreshPaused.Store(paused)
}

// SetClockRefreshPaused is an alias for SetRefreshPaused.
...
// CachedUnixMilli returns the millisecond wall clock refreshed on a 1ms ticker.
func CachedUnixMilli() int64 {
	return cachedUnixMilli.Load()
}

// RefreshWallClockNow forces a syscall refresh of cached wall-clock fields.
func RefreshWallClockNow() {
