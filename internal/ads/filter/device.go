...
type DeviceFilter struct {
	settings   *catalog.SettingsWatcher
	blockedTLS map[uint32]struct{}
	mu         sync.RWMutex
}

// NewDeviceFilter builds a device integrity gate backed by dynamic config snapshots.
func NewDeviceFilter(settings *catalog.SettingsWatcher) *DeviceFilter {
