...
func ReadBudgetInvariant(ctx context.Context, pool *pgxpool.Pool, rdb redis.Cmdable, campaignID uuid.UUID) (BudgetInvariantSnapshot, error) {
	...
	err := pool.QueryRow(ctx,
		`SELECT budget_limit, current_spend FROM campaigns WHERE id = $1`, repo.ToUUID(campaignID),
...
// AssertBudgetInvariant verifies CHAOS.md §3.1: (budget_limit - redis_remaining) = pg_current_spend + sync_delta.
