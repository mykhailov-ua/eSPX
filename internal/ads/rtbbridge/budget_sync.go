...
func SyncRTBBudgetState(
	...
	for _, camp := range campaigns {
		...
		remaining := adcatalog.RemainingBudgetMicro(camp)
...
func loadRedisDailySpend(
	...
	keyBuf = appendDailyDate(keyBuf, time.Now().In(loc))
	key := string(keyBuf)
...
func appendDailyDate(dst []byte, t time.Time) []byte {
	y, m, d := t.Date()
	dst = append(dst, byte('0'+y/1000), byte('0'+(y/100)%10), byte('0'+(y/10)%10), byte('0'+y%10), '-')
