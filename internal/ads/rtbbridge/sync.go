...
// HybridCampaignBalancer rebuilds weighted campaign selection from registry snapshots.
type HybridCampaignBalancer interface {
	UpdateCampaigns(campaigns []*CampaignMeta, secondsElapsed, totalSeconds int64)
}
...
func BuildCampaignMetaList(campaigns []*domain.Campaign, cfg *config.Config) []*CampaignMeta {
	...
	for _, camp := range campaigns {
		...
		out = append(out, &CampaignMeta{
			...
			BidMicro:          bidDefault,
			CTR:               1.0,
			RemainingBudget:   adcatalog.RemainingBudgetMicro(camp),
			TotalBudget:       total,
...
func buildCustomerBudgetPools(campaigns []*domain.Campaign) map[uuid.UUID]int64 {
	...
	for _, camp := range campaigns {
		...
		out[camp.CustomerID] += adcatalog.RemainingBudgetMicro(camp)
	}
	return out
}
