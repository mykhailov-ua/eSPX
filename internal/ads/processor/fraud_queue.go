...
// fraudValuesPool recycles Redis XADD value slices for fraud stream batches.
var fraudValuesPool = sync.Pool{
	New: func() any {
		s := make([]any, 22)
		return &s
	},
...
func (q *FraudStreamWriter) Enqueue(shard int, evt *domain.Event) bool {
	...
	spaceAvailable:
		...
		slot.payloadLen = uint16(copyFraudField(slot.payload[:], repo.UnsafeString(evt.Payload)))
...
func (q *FraudStreamWriter) drain(final bool) {
	...
	wCamp := filter.BufPool.Get().(*filter.BufWrapper)
	wTime := filter.BufPool.Get().(*filter.BufWrapper)
	defer func() {
		fraudValuesPool.Put(valSlicePtr)
		filter.BufPool.Put(wCamp)
		filter.BufPool.Put(wTime)
	}()
...
func (q *FraudStreamWriter) flushBatch(ctx context.Context, batch []*fraudStreamSlot, valSlice []any, wCamp, wTime *filter.BufWrapper) {
	if len(batch) == 0 {
		return
	}
	...
	for _, sb := range shards {
		...
		for _, cmd := range sb.cmds {
			if cmd.Err() != nil {
				metrics.FilterInternalErrors.WithLabelValues("fraud_stream_write").Inc()
			}
		}
	}
}
...
func fillFraudStreamValuesFromSlot(valSlice []any, slot *fraudStreamSlot, wCamp, wTime *filter.BufWrapper) {
	wCamp.Buf = wCamp.Buf[:0]
	wCamp.Buf = repo.AppendUUID(wCamp.Buf, slot.campaignID)
	campIDStr := repo.UnsafeString(wCamp.Buf)

	wTime.Buf = wTime.Buf[:0]
	wTime.Buf = time.Unix(0, slot.createdAt).AppendFormat(wTime.Buf, time.RFC3339Nano)
	timeStr := repo.UnsafeString(wTime.Buf)

	valSlice[0] = "click_id"
	valSlice[1] = repo.UnsafeString(slot.clickID[:slot.clickLen])
	valSlice[2] = "campaign_id"
	valSlice[3] = campIDStr
	valSlice[4] = "user_id"
	valSlice[5] = repo.UnsafeString(slot.userID[:slot.userLen])
	valSlice[6] = "type"
	valSlice[7] = repo.UnsafeString(slot.evtType[:slot.typeLen])
	valSlice[8] = "ip"
	valSlice[9] = repo.UnsafeString(slot.ip[:slot.ipLen])
	valSlice[10] = "ua"
	valSlice[11] = repo.UnsafeString(slot.ua[:slot.uaLen])
	valSlice[12] = "payload"
	valSlice[13] = repo.UnsafeString(slot.payload[:slot.payloadLen])
	valSlice[14] = "fraud_reason"
	valSlice[15] = repo.UnsafeString(slot.reason[:slot.reasonLen])
	wCamp.Buf = wCamp.Buf[:0]
	wCamp.Buf = strconv.AppendUint(wCamp.Buf, uint64(slot.fraudScore), 10)
	valSlice[16] = "fraud_score"
	valSlice[17] = repo.UnsafeString(wCamp.Buf)
...
func EnqueueFraudReject(writer *FraudStreamWriter, shard int, evt *domain.Event) {
	enqueueFraudReject(writer, shard, evt)
}
