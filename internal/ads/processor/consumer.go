...
// logBufPool is deprecated; use repo.ByteBufPool.
var logBufPool = repo.ByteBufPool

// adLogRecordPool is deprecated; use repo.AdLogRecordPool.
var adLogRecordPool = repo.AdLogRecordPool
...
func (consumer *StreamConsumer) moveToDLQ(ctx context.Context, batch []*domain.Event, msgIDs []string, workerID string, retryCount int, err error) error {
	...
	wrapPtrs := make([]*repo.ByteSliceValue, 0, len(batch))
	defer func() {
		for _, ptr := range valuesPtrs {
			dlqValuesPool.Put(ptr)
		}
		for _, ptr := range bufPtrs {
			repo.ByteBufPool.Put(ptr)
		}
		for _, ptr := range wrapPtrs {
			repo.ByteSliceValuePool.Put(ptr)
		}
	}()
...
func (consumer *StreamConsumer) parseMessage(id string, values map[string]interface{}) *domain.Event {
	...
	if rawBytesStr, ok := values["d"].(string); ok {
		...
		buf := repo.UnsafeBytes(rawBytesStr)
		if err := pbEvt.UnmarshalVT(buf); err == nil {
			...
			event.ClickID = repo.UnsafeString(event.StringBuffer[len(event.StringBuffer)-len(pbEvt.ClickId):])

			event.StringBuffer = append(event.StringBuffer, pbEvt.EventType...)
			event.Type = repo.UnsafeString(event.StringBuffer[len(event.StringBuffer)-len(pbEvt.EventType):])

			event.StringBuffer = append(event.StringBuffer, pbEvt.Ip...)
			event.IP = repo.UnsafeString(event.StringBuffer[len(event.StringBuffer)-len(pbEvt.Ip):])

			event.StringBuffer = append(event.StringBuffer, pbEvt.Ua...)
			event.UA = repo.UnsafeString(event.StringBuffer[len(event.StringBuffer)-len(pbEvt.Ua):])

			_ = repo.ParseUUID(pbEvt.CampaignId, &event.CampaignID)
			...
			if len(pbEvt.FraudReason) > 0 {
				...
				event.FraudReason = repo.UnsafeString(event.StringBuffer[len(event.StringBuffer)-len(pbEvt.FraudReason):])
			}
