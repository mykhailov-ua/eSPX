...
// attachFraudAccumulator binds a pooled accumulator to evt.HotScratch for the current Check pass.
...
// releaseFraudAccumulator clears HotScratch and returns the accumulator to fraudAccPool.
...
// fraudAccFromEvent loads the accumulator stored in evt.HotScratch during FilterEngine.Check.
func fraudAccFromEvent(evt *domain.Event) (*fraudAccumulator, bool) {
	if evt == nil {
		return nil, false
	}
	acc, ok := evt.HotScratch.(*fraudAccumulator)
	return acc, ok
}
