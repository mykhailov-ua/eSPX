...
const mlWorkerDownP99Limit = 80 * time.Millisecond
...
func TestChaos_MLWorkerDown(t *testing.T) {
	...
	require.Less(t, p99, mlWorkerDownP99Limit, "control cohort p99 must stay <80 ms when ML worker is down")

	AssertBudgetInvariant(t, ctx, infra.Pool, infra.Redis, stack.CampaignID)

	logChaosProof(t, "ml_worker_down", map[string]string{
...
func TestChaos_MLHotPathZeroAlloc(t *testing.T) {
	...
	logChaosProof(t, "ml_hotpath_zero_alloc", map[string]string{
		"subsystem": "ml_analytics",
		"allocs_op": "0",
	})
}
