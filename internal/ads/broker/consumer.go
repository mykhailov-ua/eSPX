...
func NewBrokerStreamConsumer(
	...
	return &BrokerStreamConsumer{
		...
		retryInit:    retryInit,
		retryMax:     retryMax,
		cb:           processor.NewCircuitBreaker(maxRetries, retryMax*2),
...
func (b *BrokerStreamConsumer) flushAndCommit(ctx context.Context, batch []*domain.Event, nextCommit uint64) (uint64, error) {
	if len(batch) == 0 {
...
func (b *BrokerStreamConsumer) writeAudit(evt *domain.Event) {
	...
	processor.WriteAuditLog(b.logger, &b.auditLogSeq, processor.AuditLogSampleMaskDefault, 0, evt)
}
