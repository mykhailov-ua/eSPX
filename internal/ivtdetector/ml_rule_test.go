...
func TestMLRule_Integration(t *testing.T) {
	if testing.Short() {
	...
	scorer, err := mlanalytics.NewLGBMScorer("../mlanalytics/testdata/model.txt")
	require.NoError(t, err)

	rule := NewMLRule(conn, nil, scorer, 100)
	assert.Equal(t, "ml_analytics_shadow", rule.Name())
...
func TestMLRule_FraudScoresHigherThanControl(t *testing.T) {
	if testing.Short() {
	...
	_, err = NewMLRule(conn, nil, scorer, 100).Find(ctx)
	require.NoError(t, err)
...
func (m *mockScorer) ScoreBatch(ctx context.Context, rows []mlanalytics.FeatureRow) ([]float64, error) {
	return m.scores, nil
}
...
func TestMLRule_WithCampaignThresholds(t *testing.T) {
	if testing.Short() {
		t.Skip("clickhouse integration test")
	}
	...
	rule := NewMLRule(conn, pool, scorer, 100)
