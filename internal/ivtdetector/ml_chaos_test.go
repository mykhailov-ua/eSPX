...
func TestMLRule_EmptyWindow(t *testing.T) {
	if testing.Short() {
...
func TestChaos_MLClickHouseDown(t *testing.T) {
	if testing.Short() {
	...
	rule := NewMLRule(&failingCHConn{queryErr: errors.New("clickhouse unavailable")}, nil, scorer, 100)
	...
	logChaosProof(t, "ml_clickhouse_down", map[string]string{
...
func ensureMLShadowTables(t *testing.T, conn driver.Conn) {
	t.Helper()
