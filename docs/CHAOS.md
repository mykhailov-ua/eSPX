...
Fault injection, steady-state hypotheses, budget invariants.

Runner: `scripts/chaos-drills/test_chaos.sh`. Development CI: [DEVELOPMENT.md](./DEVELOPMENT.md).
...
8. Shard 0 outage — `tests/chaos/shard_outage_chaos_test.go` — Shards 1-3 remain unaffected; shard-0 campaigns `503 shard_unavailable`.
...
**Hypothesis:** Shards 1-3 track normally; shard-0 campaigns return explicit `503 shard_unavailable`; unknown IDs in stale-serve return `503 registry_stale`; management outbox stays PENDING; budget invariant holds.  
**CI Test:** `tests/chaos/shard_outage_chaos_test.go` · Drill: `scripts/chaos-drills/m14_shard0_failure.sh`
1. Send traffic to all shards; record baseline RPS and latency.
2. Stop the `redis-0` master container; trip shard-0 breaker.
3. Verify: Shard 0 circuit breaker opens; shards 1-3 p99 latency remains within budget; outbox UPDATE_SETTINGS stays PENDING; shard-0 track body contains `shard_unavailable`.
4. Restart `redis-0`; Sentinel restores master status (or testcontainer restart in CI).
5. Verify: Breaker closes; outbox flushes pending items; shard 0 tracking recovers; `AssertBudgetInvariant` on shards 1–3.
6. Proof: `chaos_proof fault=shard_0_outage status=recovered` and `chaos_proof fault=shard0_survival_shards_1_3`.

### Scenario A2: Local Quanta Graceful Shutdown (M14-14)
**Hypothesis:** Unused RAM quanta return to Redis `budget:quota` on SIGTERM/FlushAll; no stranded under-spend beyond recon tolerance.  
**CI Test:** `internal/ingestion/local_quanta_flush_test.go`
1. Credit local ledger for N campaigns; seed Redis quota keys.
2. Call `FlushAll` (simulates tracker SIGTERM path before pin close).
3. Verify Redis balances increased by flushed amounts; ledger remaining is 0.
4. Proof: `chaos_proof fault=quanta_graceful_shutdown campaigns=<n> redis_returned=true`.
...
**Full catalog:** [BACKLOG.md](./BACKLOG.md) §9 — chaos backlog including elastic shard orchestrator scenarios:
...
Implement per [R10 #11, #13](CHAOS.md); update `CHAOS_MIN_PROOFS` when new CI proofs land.
