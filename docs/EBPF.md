...
Kernel L4 filter on tracker `:8180`. Edge: [EDGE.md](./EDGE.md). Compliance: [COMPLIANCE.md](./COMPLIANCE.md). Shipped: M10 tiers A–C. Open: tier D §7.
...
| **Stats** | `PERCPU_ARRAY` counters: pass, allow, drop blocklist/SYN/global/PPS/subnet/anomaly/RST; fingerprint + syn-cookie |
| **SYN cookies** | Tail-called `xdp_syn_cookie`; `XDP_SYN_COOKIE=1`; kernel ≥ 6.0 (`bpf_tcp_raw_gen_syncookie_ipv4`) |
| **/24 SYN cap** | `syn_subnet_ratelimit_v4` LRU — 256/s per `/24`, independent of per-host SYN |
| **Violation ringbuf** | SYN/PPS/subnet/global drops → `edge-bpf-sync` → Redis `blacklist:auto` + immediate LPM sync |
| **TCP fingerprint** | SYN-stage hash(window, MSS, doff, TTL) → ringbuf → Redis staging → `ivt-detector` (`ML_GHOST_IVT`) |
| **Loader** | `edge-xdp`: attach generic/native/offload, pin maps |
| **Sync** | `edge-bpf-sync`: Redis shard-0 → pinned LPM; violation + fingerprint ringbuf drain; license gate `ebpf_xdp_edge` |
...
| XDP benchmarks | 6 benches | `internal/edge/bpf/bench_test.go` |
| Tier B/C chaos | 15+ proofs | `internal/edge/bpf/*_tier_*_test.go`, `edge_filter_sync_chaos_test.go` |
...
| Benchmark | Path exercised | ns/op | allocs/op |
| :--- | :--- | ---: | ---: |
| `BenchmarkXDP_passSYN` | `:8180` SYN, fingerprint on | ~1013 | 0 |
| `BenchmarkXDP_passSYN_noFingerprint` | SYN, fingerprint off | ~934 | 0 |
| `BenchmarkXDP_dropBlocklist` | LPM deny hit | ~857 | 0 |
| `BenchmarkXDP_passPPSACK` | Established ACK + LRU token decrement | ~955 | 0 |
| `BenchmarkXDP_dropAnomaly` | SYN+FIN anomaly | ~858 | 0 |
| `BenchmarkXDP_dropNonTCP` | UDP to `:8180` | ~837 | 0 |

Benches use `Program.Run()` with a pre-allocated `DataOut` buffer (not `Program.Test()`, which allocates per call). **Privileged host, linux/amd64, 2026-07-24.**
...
**Status:** B1/B3/B4 done · B2 deferred (legal review)

---

## 6. Tier C — P2: passive IVT signals

**Goal:** Passive fraud signals for IVT — **score only**, not L4 hard block ([COMPLIANCE.md](./COMPLIANCE.md) §1.B).
...
| **M10-D3** | XDP chaos injector | Lab/staging BPF prog; %-drop or delay by CIDR; not enabled in production by default ([BACKLOG.md](./BACKLOG.md)) |
...
| Port scan / hack-back from `edge-bpf-sync` | CFAA; [COMPLIANCE.md](./COMPLIANCE.md) §2 |
...
Mandatory patterns for all `edge_filter.c` changes (Go analog: [GO.md](./GO.md)).
...
| [BACKLOG.md](./BACKLOG.md) | Open XDP/P3 ideas |
| [CAPABILITIES.md](./CAPABILITIES.md) | M10 capability summary |
...
| Active port scan / nmap | [COMPLIANCE.md](./COMPLIANCE.md) §2.C |
...
Cross-link: parallel with **M9** (Lua RTT) and **M5-A** (edge H2/H3) per [docs/CAPABILITIES.md](./docs/CAPABILITIES.md) and [docs/CAPABILITIES.md](./docs/CAPABILITIES.md).
