...
Defensive vs forbidden perimeter behavior. Not legal advice.

eBPF binding: [EBPF.md](./EBPF.md). Architecture: [ARCHITECTURE.md](./ARCHITECTURE.md).
...
Separate from **fraud passive telemetry** (§1.B) and **license heartbeat** (`ESPX_LICENSE_TELEMETRY` in [MANAGEMENT.md](docs/MANAGEMENT.md) §8).
...
Full spec: [BACKLOG.md](./BACKLOG.md) GAP-ENG-03 (vendor telemetry).
...
IVT/bot rules (M14) must query `ip_hash` after cutover. Erasure bumps salt + CH mutation per `ErasureWorker`.

Full spec: [BACKLOG.md](./BACKLOG.md) GAP-DATA-01 (PII in ClickHouse).
