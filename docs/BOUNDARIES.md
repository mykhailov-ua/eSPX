...
Microservice vs in-process decision rubric. Scope: `cmd/*` cold path only; not tracker hot path.

Control plane context: [MANAGEMENT.md](./MANAGEMENT.md).
...
6. Compose entry with memory limits; ports documented in `ARCHITECTURE.md`.
...
- `ARCHITECTURE.md` — Topology, DB schemas, settlement flows.
