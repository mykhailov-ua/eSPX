...
func TestSegmentRoll_CrossSegmentFetch(t *testing.T) {
	...
	topicDir := filepath.Join(dir, topic, "0")
...
func TestMalformedFrames_ServerDoesNotPanic(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			frame: func() []byte {
				inner := protocol.EncodeProduceRequest(make([]byte, 1024), 1, "", 0, []byte("x"))
				return inner
			}(),
		},
		{
...
func TestBrokerPipeAllocs(t *testing.T) {
	...
	go func() {
		defer close(done)
		for {
			_, _, reqPayload, err := protocol.ReadFrame(c2, readBuf, lenBuf)
			if err != nil {
				return
			}
			_, _, msg, err := protocol.DecodeProduceRequest(reqPayload)
			if err != nil {
				return
			}
			_ = msg
			resp := protocol.EncodeProduceResponse(writeBuf, 1, 0, 0)
			_, _ = c2.Write(resp)
		}
	}()
	...
	allocs := testing.AllocsPerRun(1000, func() {
		seq++
		req := protocol.EncodeProduceRequest(writeBuf, seq, topic, 0, payload)
		_, _ = c1.Write(req)
		_, _, _, _ = protocol.ReadFrame(c1, readBuf, lenBuf)
	})
	_ = c2.Close()
	<-done
...
func TestBrokerE2EAllocs(t *testing.T) {
	...
	if fetchAllocs > 1 {
		t.Fatalf("Fetch: got %v allocs/op, want <=1", fetchAllocs)
	}
}
