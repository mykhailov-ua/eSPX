# Guide: Code Style and Repository Structure
...
Standardize the repository structure, flat packages, struct roles, boundary mapping, naming, comments, and review gates to ensure services remain easy to navigate without nested domain packages. Hot-path rules in `.cursorrules` take precedence over idiomatic Go in case of conflict.

## Scope

**In Scope:**
1. Repository structure, flat packages, struct roles, and boundary mapping.
2. File naming and imports in `internal/<service>/`.
3. Comments, error handling, and blank identifier usage (R7, R8).
4. Hot-path performance and cold-path idiomatic Go (R8.6, R8.7).
5. Tooling, codegen hooks, and CI gates (R9.1, R11).

**Out of Scope:**
1. Protobuf/sqlc code generation mechanics (plugin details — R11; commands — `make proto`).
2. Non-Go assets (Lua, SQL), except for their location.
3. Complete YAML workflow definitions (links to `.github/workflows/` — R11).
